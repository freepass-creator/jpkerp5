...
/**
 * 모바일 레이아웃 — /m/* 별도 트리. 6탭 하단 고정.
 *  · 홈(검색) / 운영 / 리스크 / 입력 / 업로드 / 설정
 *  · safe-area 대응 (iOS 노치/홈바)
 */

import Link from 'next/link';
import { usePathname } from 'next/navigation';
import { House, ChartLineUp, Warning, NotePencil, UploadSimple, GearSix } from '@phosphor-icons/react';
...
const TABS: { href: string; label: string; icon: (p: { size: number; weight: 'duotone' | 'bold' }) => JSX.Element; match: (p: string) => boolean }[] = [
  { href: '/m',         label: '홈',     icon: ({ size, weight }) => <House size={size} weight={weight} />,
    match: (p) => p === '/m' },
  { href: '/m/ops',     label: '운영',   icon: ({ size, weight }) => <ChartLineUp size={size} weight={weight} />,
