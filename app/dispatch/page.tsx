 ...
 * 디스패치 (업무 지시) — 데스크탑.
 *
 * 사무에서 현장 직원에게 업무 지시 등록 + 전체 지시 이력 모니터링.
 * 모바일 /m/orders 에서 받기 + 사무에서 새 지시 보내기 (모달).
 */
...
import {
  createDispatchOrder, updateDispatchStatus,
  DISPATCH_LABEL,
      ...
      <div className="app">
        <header className="topbar">
          <div className="topbar-title">
            ...
            <span style={{ color: 'var(--text-main)', fontWeight: 600 }}>업무 지시</span>
          </div>
        </header>
        ...
        <DialogBody style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <Field label="제목 *">
            <input value={title} onChange={(e) => setTitle(e.target.value)}
              placeholder="간단한 제목" className="input" style={{ width: '100%' }} />
          </Field>
